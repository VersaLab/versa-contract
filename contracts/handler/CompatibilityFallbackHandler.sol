...
contract CompatibilityFallbackHandler is TokenCallbackHandler, IERC1271 {
     ...
     */
    function isValidSignature(bytes32 _hash, bytes calldata _signature) public view override returns (bytes4 magicValue) {
        address validator = address(bytes20(_signature[0:20]));
        require(ValidatorManager(msg.sender).getValidatorType(validator) == ValidatorManager.ValidatorType.Sudo, "Only sudo validator");
