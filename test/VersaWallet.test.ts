...
describe('VersaWallet', () => {
  ...
  it('should valdiate userOp correctly', async () => {
    ...
    let sudoExecuteData = wallet.interface.encodeFunctionData("sudoExecute", [
        owner.address, parseEther("0.1"), '0x', 0
    ])
    let op = {
  ...
  it('should execute', async () => {
    ...
    let sudoExecuteData = wallet.interface.encodeFunctionData("sudoExecute", [
        owner.address, parseEther("1"), '0x', 0
    ])
    let op = {
        sender: wallet.address,
        nonce: 0,
  ...
  it("should revert if validator and selector don't match", async () => {
    ...
    let opHash = await opHasher.getUserOpHash(op)
    await expect(wallet.connect(entryPoint).validateUserOp(op, opHash, parseEther("0.1")))
      .to.be.revertedWith("Versa: selector doesn't match validator")
    })
    ...
    it("should revert if normal execution uses banned operation", async () => {
        ...
        // Perform a self call
        await expect(wallet.connect(entryPoint).normalExecute(wallet.address, parseEther("0.1"), '0x', 0))
          .to.be.revertedWith("Versa: operation is not allowed")
        
        // Use delegatecall
        await expect(wallet.connect(entryPoint).normalExecute(ethers.constants.AddressZero, parseEther("0.1"), '0x', 1))
        .to.be.revertedWith("Versa: operation is not allowed")

        // Call to enabled plugin
        await expect(wallet.connect(entryPoint).normalExecute(module.address, parseEther("0.1"), '0x', 1))
        .to.be.revertedWith("Versa: operation is not allowed")

        // Call to enabled plugin
        await expect(wallet.connect(entryPoint).normalExecute(sudoValidator.address, parseEther("0.1"), '0x', 1))
        .to.be.revertedWith("Versa: operation is not allowed")

        // Call to enabled plugin
        await expect(wallet.connect(entryPoint).normalExecute(hooks.address, parseEther("0.1"), '0x', 1))
        .to.be.revertedWith("Versa: operation is not allowed")
    })
});
