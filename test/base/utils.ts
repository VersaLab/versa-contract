...
export async function execute(options: {
    ...
    const { executor, to = executor.address, value = parseEther("0"), data = "0x", operation = 0 } = options;
...
export async function disablePlugin(executor: MockHooksManager | any, plugin: string) {
    let data;
    if (isHooksManager(executor)) {
...
export async function toggleValidator(executor: MockValidatorManager, validator: string) {
...
function isVersaWallet(value: any): value is VersaWallet {
    return typeof value === "object" && value !== null && "sudoExecute" in value;
}

function getPrevPlugin(plugin: string, list: string[]) {
