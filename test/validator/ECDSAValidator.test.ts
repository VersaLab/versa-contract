...
describe('ECDSAValidator', () => {
  ...
  it('should fail when validator is an EOA', async () => {
    await expect(ecdsaValidator.connect(signer1).setSigner(signer1.address))
      .to.be.revertedWithoutReason() // without reason because signer1 is an EOA account and doesn't have
